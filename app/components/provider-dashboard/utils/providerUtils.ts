...
export const getProviderDashboardStats = (
  ...
  const ratings = services.filter((s) => s.rating > 0).map((s) => s.rating);
  const averageRating =
    ratings.length > 0
...
export const getUpcomingBookings = (bookings: ProviderBooking[]): ProviderBooking[] => {
  ...
  return bookings
    ...
    .sort((a, b) => {
      const dateA = new Date((a.rescheduleDate || a.date) + "T" + (a.rescheduleTime || a.time));
      const dateB = new Date((b.rescheduleDate || b.date) + "T" + (b.rescheduleTime || b.time));
      return dateA.getTime() - dateB.getTime();
    });
};

export const getPastBookings = (bookings: ProviderBooking[]): ProviderBooking[] => {
  return bookings
...
export const filterAndSortBookings = (
  bookings: ProviderBooking[],
  filters: { status: string; dateRange: string; serviceId: string; searchQuery: string }
): ProviderBooking[] => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  ...
  return bookings
    .filter((b) => {
      ...
      if (filters.dateRange !== "all") {
        ...
        if (filters.dateRange === "today" && d.toDateString() !== today.toDateString()) return false;
